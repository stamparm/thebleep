import pytest
from thebleep.rules.git_rm_local_modifications import match, get_new_command
...
def test_rm_text_in_another_command_is_not_a_rm():
    output = ('error: the following file has local modifications: foo\n'
...
def test_get_new_command_preserves_quoted_filename():
    output = ('error: the following file has local modifications: foo bar\n'
              '(use --cached to keep the file, or -f to force removal)')
