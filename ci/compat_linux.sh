...
#
#     ci/compat_linux.sh IMAGE LABEL OUTDIR [checkout|pypi]
#
...
case "${ID_LIKE:-} $ID" in
    *debian*|*ubuntu*)
        ...
        apt-get install -y -qq --no-install-recommends \
            python3 python3-venv python3-pip git sudo ca-certificates \
            >/dev/null ;;
    *fedora*|*rhel*|*centos*)
...
su bleeper -s /bin/sh -c \
    "sh /tmp/src/ci/compat_unix.sh '$LABEL' /tmp/src /out '$SOURCE'"
INSIDE
