...
outages, changed SSH host keys, existing paths, read-only filesystems, missing
...
Justfile or Cargo.toml when one is available. The npm and bun rules use declared
script names directly, while `make_no_target`, `just_no_recipe` and
`cargo_no_bin` use static task or binary names, so correction does not launch a
project tool just to list vocabulary. These files are bounded and never
executed; an unreadable source leaves the existing tool-output fallback or
abstains safely.
...
$ thebleep --doctor
  ...
  Rules               189 bundled, 3 of your own
  Rule health         169 enabled, none raising
  Rule pack           ~/.cache/thebleep/rules-3-cb0d0d0a.pack (189 rules cached)
...
| **Rules** | 189 of them, for git, docker, npm, yarn, pip, apt, dnf, zypper, pacman, brew, cargo, go, gradle, maven, terraform, aws, az, systemctl and the rest |
...
- **Most rules are never loaded.** A rule that declares `@for_app('git', ...)`,
  ...
  running it. A typical command now reaches about a fifth of the 189 rules, and
