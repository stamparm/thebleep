...
def _cmake_lists(start=None):
    """Return the nearest CMakeLists.txt, if one exists."""
    return find_up('CMakeLists.txt', start)


def _read_cmake_lists(path):
    try:
        if os.path.getsize(path) > MAX_CMAKE_BYTES:
            return None
        with open(path, encoding='utf-8') as handle:
            value = handle.read(MAX_CMAKE_BYTES + 1)
    except (EnvironmentError, UnicodeError):
        return None

    return value if len(value.encode('utf-8')) <= MAX_CMAKE_BYTES else None


_CMAKE_TARGET = re.compile(
    r'^\s*add_(?:custom_target|executable|library)\s*\(\s*'
    r'(?:(?:"([^"]+)"|\'([^\']+)\')|([A-Za-z0-9_.:+/-]+))',
    re.IGNORECASE)


def cmake_targets(start=None):
    """Return statically named targets declared in CMakeLists.txt.

    CMake is a language, not a data format: targets can be generated by
    functions, variables and included files. This deliberately reads only the
    first argument of common target declarations in one bounded file; dynamic
    names and other CMake constructs are left to CMake itself.
    """
    path = _cmake_lists(start)
    if path is None:
        return None

    source = _read_cmake_lists(path)
    if source is None:
        return None

    found = []
    for line in source.splitlines():
        match = _CMAKE_TARGET.match(line)
        if not match:
            continue
        name = next(value for value in match.groups() if value is not None)
        if _just_name(name) and name not in found:
            found.append(name)
