...
| `docker pss` | `docker pss` | ✓ `docker ps` |
| `npm run bulid` | `npm run bulid` | ✓ `npm run build` |
| `cargo biuld` | `cargo biuld` | ✗ nothing suggested |
...
| `sl -la` | `sl -la` | ✓ `ls -la` |
| `git status --shrot` | `git status --shrot` | ✓ `git status --short` |
| `apt isntall`, `dnf`, `apk`… | `pkg_add isntall` | ✓ `sudo pkg_add isntall` |
...
Command-only correction in 160 ms, median of 5 runs, Python start included.
...
| `sl -la` | `sl -la` | ✓ `ls -la` |
| `git status --shrot` | `git status --shrot` | ✓ `git status --short` |
| `apt isntall`, `dnf`, `apk`… | `pkg_add isntall` | ✗ error: could not run the shell: Command '['bash', '-c', 'pkg_add isntall 2>&1']' timed out after 10 seconds |
| permission denied | `cat /etc/master.passwd` | ✓ `sudo cat /etc/master.passwd` |
| `mkdir a/b/c` | `mkdir a/b/c` | ✓ `mkdir -p a/b/c` |
| `./script.sh` without +x | `./script.sh` | ✓ `chmod +x script.sh && ./script.sh` |
| `cd Documnets` | `cd Documnets` | ✓ `cd Documents` |
...
Command-only correction in 204 ms, median of 5 runs, Python start included.
...
|---|---|---|
| `gti status` | `gti status` | ✓ `git status` |
| `git psuh` | `git psuh` | ✗ suggested `git 'push'` |
| `sl -la` | `lss` | ✓ `ls` |
| `git status --shrot` | `git status --shrot` | ✗ suggested `git status '--short'` |
| `apt isntall`, `dnf`, `apk`… | `winget isntall vim` | ✗ nothing suggested |
...
| `cd Documnets` | `cd Documnets` | ✗ suggested `cd 'Documents'` |
...
| `docker pss` | `docker pss` | ✗ suggested `docker 'ps'` |
| `npm run bulid` | `npm run bulid` | ✗ suggested `npm run 'build'` |
| `cargo biuld` | `cargo biuld` | ✗ nothing suggested |
